                ...
                try {
                    ...
                    if (toolCall.function.name === 'buat_link_pembayaran_dp') {
                        try {
                            ...
                            // ── Coba buat QRIS via Xendit /qr_codes ─────────────
                            let qrisResult = null;
                            const xenditSvc = getXenditService();

                            if (xenditSvc?.createQrisPayment) {
                                try {
                                    const referenceId = `QRIS-${tipeBayar}-${customerPhone || 'noPhone'}-${Date.now()}`;
                                    qrisResult = await xenditSvc.createQrisPayment({
                                    ...
                                    logger.warn(`[AI] Xendit createQrisPayment error: ${xenErr.message}`);
                                }
                            }

                            if (qrisResult?.success && qrisResult?.qrisImageBuffer) {
                                // ── Sukses: kirim gambar QRIS PNG ke WA customer ──
                                try {
                                    const waSvc = require('./whatsapp_service');
                                    if (storeWaId && customerPhone && typeof waSvc.sendQrisImage === 'function') {
                                        // Jika ada fungsi khusus kirim QRIS image
                                        await waSvc.sendQrisImage(storeWaId, customerPhone, qrisResult.qrisImageBuffer, [
                                            `Ini QRIS pembayarannya ya bund 😊`,
                                            `Nominal: *Rp ${amount.toLocaleString('id-ID')}* [${tipeBayar}]`,
                                            `Tinggal scan dari m-banking bund, berlaku 30 menit ya 🙏`,
                                        ].join('\n\n'));
                                        logger.info(`[AI] ✅ QRIS PNG terkirim ke ${customerPhone} via WA`);
                                    } else if (storeWaId && customerPhone && typeof waSvc.sendFollowUpMessage === 'function') {
                                        // Fallback: kirim via sendFollowUpMessage dengan MessageMedia
                                        const { MessageMedia } = require('whatsapp-web.js');
                                        const client = waSvc.getActiveClient ? waSvc.getActiveClient(storeWaId) : null;
                                        if (client) {
                                            const media = new MessageMedia('image/png', qrisResult.qrisImageBuffer.toString('base64'), 'qris_payment.png');
                                            const caption = [
                                                `Ini QRIS pembayarannya ya bund 😊`,
                                                `Nominal: *Rp ${amount.toLocaleString('id-ID')}* [${tipeBayar}]`,
                                                `Tinggal scan dari m-banking bund, berlaku 30 menit ya 🙏`,
                                            ...
                                            const msgId = msg?.id?._serialized || msg?.id?.id;
                                            if (msgId && waSvc.trackBotSentMessage) waSvc.trackBotSentMessage(msgId);
                                            logger.info(`[AI] ✅ QRIS PNG terkirim ke ${customerPhone} via client.sendMessage`);
                                        }
                                    }
                                } catch (waErr) {
                                    logger.error(`[AI] Gagal kirim QRIS PNG ke WA: ${waErr.message}`);
                                    // Non-fatal: lanjutkan, AI tetap kasih info ke customer
                                }
                            ...
                            } else if (qrisResult?.success && qrisResult?.qr_string) {
                                // QRIS berhasil tapi buffer tidak ada — kasih info ke AI
                                messages.push({
                                    tool_call_id: toolCall.id, role: 'tool',
                                    name: 'buat_link_pembayaran_dp',
                                    content: [
                                ...
                                // Xendit tidak tersedia / gagal → fallback ke transfer manual
                                messages.push({
                                    tool_call_id: toolCall.id, role: 'tool',
                                    name: 'buat_link_pembayaran_dp',
                                    content: [
                            ...
                            logger.error(`[AI] buat_link_pembayaran_dp error: ${xenToolErr.message}`);
                            messages.push({
                                tool_call_id: toolCall.id, role: 'tool',
                                name: 'buat_link_pembayaran_dp',
                                content: `Sistem pembayaran sementara tidak tersedia. Minta customer transfer manual ke rekening toko (ambil dari product knowledge). Sampaikan dengan ramah.`
