...
function setupEventListeners(client, storeWaId, io) {
    ...
    client.on('message_create', async (message) => {
        ...
        try {
            ...
            try {
                const { cancelPendingFollowUps } = require('./services/followup.service');
...
async function sendQrisImage(storeWaId, contactId, imageBuffer, caption = '') {
    ...
    try {
        ...
        const base64Data = Buffer.isBuffer(imageBuffer)
            ...
            : String(imageBuffer);
        const media = new MessageMedia('image/png', base64Data, 'qris_pembayaran.png');
        const msg = await client.sendMessage(targetChatId, media, { caption: caption || '' });
        const msgId = msg?.id?._serialized || msg?.id?.id;
        trackBotSentMessage(msgId);
        ...
        // TIDAK pause AI
        logger.info(`[${storeWaId}] QRIS image berhasil dikirim ke [${targetChatId}].`);
        return true;
    } catch (error) {
        logger.error(`[${storeWaId}] Gagal kirim QRIS image ke [${targetChatId}]: ${error.message}`);
        throw error;
    }
}
...
async function promoteTempClient(tempId, waId, io) {
    ...
    if (fs.existsSync(newSessionDir)) {
        try { fs.rmSync(newSessionDir, { recursive: true, force: true }); } catch (e) {
            logger.warn(`[${tempId}] Gagal hapus session lama ${waId}: ${cleanErrorMessage(e)}`);
        }
    ...
    if (fs.existsSync(oldSessionDir)) {
        try {
            ...
            try {
                ...
                logger.error(`[${tempId}] Gagal copy session: ${cleanErrorMessage(cpErr)}`);
            }
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                try {
                    if (fs.existsSync(oldSessionDir)) {
                        ...
                        logger.info(`[${tempId}] Old session dihapus (attempt ${attempt + 1})`);
                        break;
                    }
                } catch (rmErr) {
                    if (attempt < 2) {
                        logger.warn(`[${tempId}] Retry hapus old session (${attempt + 1}/3): ${cleanErrorMessage(rmErr)}`);
                        await sleep(1000);
                    } else {
                        logger.error(`[${tempId}] Gagal hapus old session setelah 3x: ${cleanErrorMessage(rmErr)}`);
                    }
                }
            }
        }
    ...
    try {
        ...
        logger.error(`[${waId}] Gagal initialize permanent client: ${cleanErrorMessage(e)}`);
...
async function destroyTempClient(tempId) {
    ...
    try {
        ...
        logger.warn(`[${tempId}] Destroy temp client error: ${cleanErrorMessage(e)}`);
    }
