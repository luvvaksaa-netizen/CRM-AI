  ...
  try {
    ...
    res.json({
      store: {
        ...
        agent: (store as any).BotAgent
          ? {
              id: (store as any).BotAgent.id,
              name: (store as any).BotAgent.name,
            }
          : null,
      },
      pendingFollowUps,
...
export const getAllStoresStatus = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
    ...
    const result = await Promise.all(
      stores.map(async (store: any) => {
        const [pendingCount, activeContacts] = await Promise.all([
          FollowUp.count({
            where: { store_wa_id: (store as any).wa_id, status: "pending" },
          }),
          ChatMessage.count({
            where: {
...
/**
 * Get WA connection status for a specific store
 * Used by frontend to detect bot reconnects and show warning banners
 */
