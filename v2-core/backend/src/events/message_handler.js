...
} = require("../utils/contact_identity");
...
async function _loadPausedFromDB() {
  try {
    const { PausedContact } = require("../models/index");
    const records = await PausedContact.findAll();
    records.forEach((r) =>
      pausedContacts.add(`${r.store_wa_id}_${r.contact_id}`),
    );
    if (records.length > 0) {
      logger.info(
        `[Pause] Dipulihkan ${records.length} kontak yang dipause dari database.`,
      );
    }
  } catch (e) {
...
async function pauseBotForContact(storeWaId, contactId) {
  ...
  try {
    ...
    /* DB write fail is non-critical */
  }
  logger.info(
    `[${storeWaId}] Bot DIPAUSE secara manual untuk kontak: ${contactId}`,
  );
}
...
async function resumeBotForContact(storeWaId, contactId) {
  ...
  try {
    ...
    /* DB delete fail is non-critical */
  }
  logger.info(
    `[${storeWaId}] Bot DIAKTIFKAN KEMBALI untuk kontak: ${contactId}`,
  );
}
...
const THINKING_DELAY_MIN_MS = Number(
  process.env.AI_THINKING_DELAY_MIN_MS || 80,
);
const THINKING_DELAY_JITTER_MS = Number(
  process.env.AI_THINKING_DELAY_JITTER_MS || 200,
);
const BETWEEN_BUBBLE_DELAY_MS = Number(
  process.env.AI_BETWEEN_BUBBLE_DELAY_MS || 200,
);
const BETWEEN_MEDIA_DELAY_MS = Number(
  process.env.AI_BETWEEN_MEDIA_DELAY_MS || 500,
);
const MEDIA_STABILITY_DELAY_MS = Number(
  process.env.WA_MEDIA_STABILITY_DELAY_MS || 250,
);
const WA_TYPING_PULSE_MS = Number(process.env.WA_TYPING_PULSE_MS || 5000);
const WA_TYPING_HARD_STOP_MS = Number(
  process.env.WA_TYPING_HARD_STOP_MS || 7000,
);
const WA_SEND_RETRY_DELAY_MS = Number(
  process.env.WA_SEND_RETRY_DELAY_MS || 1200,
);
...
// Timestamp saat server pertama kali start — dipakai untuk menolak AI reply ke pesan lama
// yang dikirim customer saat bot sedang mati/restart.
const _SERVER_START_TIME_MS = Date.now();
const _OLD_MESSAGE_GRACE_MS = 5 * 60 * 1000; // 5 menit toleransi (pesan < 5 menit sebelum start = masih dilayani)
...
function _mergeReplyBatches(existing, incoming) {
  ...
  return {
    ...
    mediaContexts: [
      ...(existing.mediaContexts || []),
      ...(incoming.mediaContexts || []),
    ],
...
function _quoteContextFromMessage(message, fallbackSender = "") {
  const quotedBody = _clipQuoteBody(
    message?.body ||
      message?.caption ||
      message?.quotedBody ||
      message?.quoted_body ||
  ...
  return {
    ...
    quoted_sender_name:
      fallbackSender || (message?.fromMe ? "Admin/AI" : "Pelanggan"),
  };
}
...
function _isVideoMediaAsset(mediaAsset = {}) {
  ...
  return (
    ...
    [".mp4", ".mov", ".avi", ".mkv", ".3gp"].includes(ext)
  );
}
...
async function _extractQuotedContext(message, storeWaId) {
  try {
    ...
    if (
      ...
      if (quoted) {
        return _quoteContextFromMessage(
          quoted,
          quoted.fromMe ? "Admin/AI" : "Pelanggan",
        );
      }
    }
    ...
    if (directQuotedId || directQuotedBody) {
      return {
        ...
        quoted_from_me:
          message?.quotedFromMe ?? message?.quoted_from_me ?? null,
        quoted_sender_name:
          message?.quotedSenderName || message?.quoted_sender_name || null,
      };
    }
  } catch (error) {
    logger.warn(
...
async function handleMessage(message, storeWaId, shouldAIReply = true) {
  ...
  if (msgId) {
    if (processedIncomingMsgIds.has(msgId)) {
      logger.info(
        `[${storeWaId}] Mengabaikan pesan duplikat (memory cache): ${msgId}`,
      );
      return;
    }
    ...
    try {
      ...
      if (existing) {
        logger.info(
          `[${storeWaId}] Mengabaikan pesan duplikat (DB cache): ${msgId}`,
        );
        return;
      }
    } catch (e) {
  ...
  // FIREWALL 0b: Tolak AI reply untuk pesan lama saat server baru restart
  // Pesan yang dikirim sebelum server start - grace period = context sudah expired
  // Tetap simpan ke DB untuk histori CS, tapi jangan balas ngawur dengan context kosong.
  if (shouldAIReply) {
    const msgTimestampMs = (message.timestamp || 0) * 1000;
    const isOldMessage =
      msgTimestampMs > 0 &&
      msgTimestampMs < _SERVER_START_TIME_MS - _OLD_MESSAGE_GRACE_MS;
    if (isOldMessage) {
      const msgAge = Math.round((Date.now() - msgTimestampMs) / 60000);
      logger.info(
        `[${storeWaId}] Pesan lama (${msgAge} menit lalu) dari [${message.from}] — disimpan ke DB, AI skip (bot baru restart).`,
      );
      shouldAIReply = false;
    }
  }
  ...
  try {
    ...
    if (message.hasMedia && !isSticker) {
      ...
      if (!shouldAIReply) {
        ...
        if (!media) {
          logger.warn(
            `[${storeWaId}] Media gagal diunduh (mungkin sudah dihapus). Lanjut proses teks.`,
          );
        } else {
          // A. FOTO (Vision)
          if (media.mimetype.startsWith("image/")) {
            logger.info(
              `[${storeWaId}] Menerima foto dari pelanggan. Menganalisis...`,
            );
          ...
          if (media.mimetype.startsWith("audio/")) {
            ...
            const audioExt =
              media.mimetype.split("/")[1].split(";")[0] || "ogg";
            ...
            if (transcription) {
              logger.success(
                `[${storeWaId}] AI "mendengar" VN: ${transcription}`,
              );
    ...
    try {
      ...
      logger.warn(
        `[${storeWaId}] Gagal membaca profil kontak [${contactId}]: ${e.message}`,
      );
    }
    let resolvedPhone = "";
    if (contactId.endsWith("@lid") && message.client) {
      try {
        let resolved = await resolvePhoneForChatId(
          message.client,
          contactId,
          storeWaId,
        );
      ...
      if (!resolvedPhone) {
        setTimeout(async () => {
          try {
            let finalResolved = await resolvePhoneForChatId(
              message.client,
              contactId,
              storeWaId,
            );
            if (!finalResolved?.phone) {
              ...
              if (finalContact && finalContact.number) {
                finalResolved = {
                  phone: finalContact.number,
                  contact: finalContact,
                };
              }
            }
            if (finalResolved && finalResolved.phone) {
              dashboard.updateContactPhoneIdentity(
                storeWaId,
                contactId,
                finalResolved,
              );
    ...
    try {
      ...
      logger.warn(
        `[${storeWaId}] buildContactIdentity gagal [${contactId}]: ${identityErr.message}`,
      );
    ...
    try {
      const storeCheck = await Store.findOne({
        where: { wa_id: storeWaId },
        attributes: ["is_bot_active", "agent_id"],
      });
      if (!storeCheck || storeCheck.is_bot_active === false) {
        ...
        logger.info(
          `[${storeWaId}] Bot NON-AKTIF (FIREWALL 3). Pesan dari ${logDisplay} dicatat, AI tidak membalas dan tidak melabeli.`,
        );
      ...
      logger.warn(
        `[${storeWaId}] FIREWALL 3 DB check gagal: ${fwErr.message}. Mengasumsikan bot aktif.`,
      );
    }
    ...
    try {
      ...
      /* Non-critical: follow-up cancel failure */
    }
    ...
    if (!shouldAIReply) {
      logger.info(
        `[${storeWaId}] Jalur Sinkronisasi: Pesan dicatat, AI Dilewati.`,
      );
      _cleanupTempFile(tempPath, storeWaId);
      return;
    }
    ...
    if (pausedContacts.has(debounceKey)) {
      logger.info(
        `[${storeWaId}] Bot sedang dipause (Human Override) untuk: ${logDisplay}`,
      );
      _cleanupTempFile(tempPath, storeWaId);
      return;
    }
    ...
    const timerId = setTimeout(async () => {
      ...
      try {
        ...
        logger.error(
          `[${storeWaId}] Debounced AI Error [${contactId}]: ${err.message}`,
        );
      } finally {
    ...
    logger.error(
      `[${storeWaId}] Gagal memproses [${contactId}]: ${error.message}`,
    );
    _cleanupTempFile(tempPath, storeWaId);
  }
}
...
async function _processAIReply(storeWaId, contactId, batch) {
  ...
  if (activeAIReplies.has(replyKey)) {
    queuedAIReplyBatches.set(
      replyKey,
      _mergeReplyBatches(queuedAIReplyBatches.get(replyKey), batch),
    );
    const now = Date.now();
    const lastLogAt = coalescedReplyLogAt.get(replyKey) || 0;
    if (now - lastLogAt > 5000) {
      logger.info(
        `[${storeWaId}] AI masih membalas [${contactId}]. Batch baru digabung ke antrean lanjutan, bukan dibuat menunggu sendiri.`,
      );
  ...
  while (activeAIReplies.has(replyKey)) {
    if (Date.now() - startWait > MAX_WAIT_MS) {
      ...
      logger.warn(
        `[${storeWaId}] Paksa hapus lock AI yang macet untuk [${contactId}] (timeout 3 menit).`,
      );
      activeAIReplies.delete(replyKey);
      break;
    }
    logger.info(
      `[${storeWaId}] AI reply untuk [${contactId}] masih berjalan. Menahan batch baru agar tidak spam.`,
    );
  ...
  try {
    ...
    while (currentBatch) {
      try {
        ...
        logger.error(
          `[${storeWaId}] AI reply gagal untuk [${contactId}]: ${err.message}`,
        );
      }
      ...
      if (currentBatch) {
        logger.info(
          `[${storeWaId}] Memproses batch lanjutan yang sudah digabung untuk [${contactId}].`,
        );
      }
    }
  } finally {
...
async function _processAIReplyUnlocked(storeWaId, contactId, batch) {
  ...
  if (!store || store.is_bot_active === false) {
    logger.info(
      `[${storeWaId}] Bot NON-AKTIF untuk Toko ini. Mengirim sinyal pembaruan rekap secara background...`,
    );
    try {
      const {
  ...
  if (!agent) {
    logger.warn(
      `[${storeWaId}] Perangkat ini belum terikat ke Agen AI manapun.`,
    );
    return;
  }
  ...
  try {
    chat =
      typeof lastMessage.getChat === "function"
        ? await lastMessage.getChat()
        : null;
  } catch (error) {
    logger.warn(
      `[${storeWaId}] Gagal membaca chat aktif [${contactId}]: ${error.message}`,
    );
  }
  let stopTyping = () => {};
  ...
  try {
    ...
    const agentAssets = await MediaAsset.findAll({
    ...
    for (const asset of agentAssets) {
      ...
      const keywords = asset.trigger_words
        .split(",")
    ...
    if (triggeredAsset) {
      logger.success(
        `[${storeWaId}] Keyword trigger [${agent.name}] aktif. Mengirim Katalog via Autopilot!`,
      );
    ...
    await new Promise((r) =>
      setTimeout(
        r,
        Math.floor(Math.random() * THINKING_DELAY_JITTER_MS) +
    ...
    stopTyping = _startTypingHeartbeat(
      chat,
      storeWaId,
      contactId,
      lastMessage.client,
      Math.max(WA_TYPING_HARD_STOP_MS, 15000),
    );
    ...
    const customerPhone =
      summaryRecord?.contact_phone ||
      (contactId ? contactId.replace("@c.us", "").replace("@lid", "") : "");
    ...
    const fallbackContent =
      aiResult.content ||
      "Mohon maaf, saya sedang kesulitan memproses pesan Anda. Bisa diulangi pertanyaannya Kak?";
    ...
    const targetTypingDelay = Math.min(
      calculateTypingDelay(primaryTextForDelay),
      4500,
    );
      ...
      for (const tc of aiResult.tool_calls) {
        if (tc.function.name === "tambahkan_label_chat") {
          try {
            ...
            for (const labelName of labelNames) {
              ...
              logger.success(
                `[${storeWaId}] AI otomatis melabeli '${labelName}' untuk [${contactId}]`,
              );
            }
          } catch (e) {
        ...
        if (tc.function.name === "matikan_bot_kontak") {
          try {
            ...
            logger.info(
              `[${storeWaId}] AI mem-pause bot untuk [${contactId}]: ${args.reason || "perlu CS manusia"}`,
            );
          } catch (e) {
    ...
    try {
      ...
      if (pausedContacts.has(`${storeWaId}_${contactId}`)) {
        logger.warn(
          `[${storeWaId}] HARD STOP: Pengiriman pesan AI digugurkan karena Toggle dimatikan secara manual oleh CS untuk [${contactId}]`,
        );
        return;
      }

      if (
        aiResult.type === RESPONSE_TYPE.MEDIA &&
        aiResult.mediaList &&
        aiResult.mediaList.length > 0
      ) {
        const hasVideo = aiResult.mediaList.some((item) =>
          _isVideoMediaAsset(item.media),
        );
        let textAlreadySent = false;

        if (aiResult.content && hasVideo) {
          await _sendTextBubbles(
            lastMessage,
            chat,
            storeWaId,
            contactId,
        ...
        for (let i = 0; i < aiResult.mediaList.length; i++) {
          ...
          if (pausedContacts.has(`${storeWaId}_${contactId}`)) {
            logger.warn(
              `[${storeWaId}] HARD STOP: Pengiriman media AI digugurkan di tengah jalan.`,
            );
            return;
          }
      ...
      logger.success(
        `[${storeWaId}] Sesi [${contactId}] — Dibalas via AI (${messages.length} pesan digabung).`,
      );
    } catch (sendErr) {
      logger.error(
        `[${storeWaId}] Gagal mengirim balasan AI ke [${contactId}]: ${sendErr.message}`,
      );
    }
...
async function _sendActiveMessage(storeWaId, contactId, payload, options = {}) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try {
      ...
      logger.warn(
        `[${storeWaId}] Client WA sempat detach saat kirim ke [${contactId}], mencoba ulang dengan client aktif...`,
      );
...
async function _markTyping(chat, storeWaId, contactId, client) {
  try {
    ...
    const markedByWajs = await safeMarkIsComposing(
      client,
      contactId,
      6000,
      storeWaId,
    ...
    logger.warn(
      `[${storeWaId}] Gagal menampilkan typing untuk [${contactId}]: ${e.message}`,
    );
    return false;
  }
}

function _startTypingHeartbeat(
  chat,
  storeWaId,
  contactId,
  client,
  hardStopMs = WA_TYPING_HARD_STOP_MS,
) {
  let stopped = false;
  const pulse = async () => {
    if (stopped) return;
 ...
 */
async function _sendTextBubbles(
  message,
  chat,
  storeWaId,
  contactId,
  ...
  const safeList =
    list.length > 0 ? list : ["Mohon maaf kak, bisa diulangi sebentar?"];
  const debounceKey = `${storeWaId}_${contactId}`;
  ...
  for (let i = 0; i < safeList.length; i++) {
    ...
    if (pausedContacts.has(debounceKey)) {
      logger.warn(
        `[${storeWaId}] HARD STOP: Bubble ke-${i + 1} digugurkan karena bot dipause untuk [${contactId}]`,
      );
      return;
    }
    ...
    try {
      const storeCheck = await Store.findOne({
        where: { wa_id: storeWaId },
        attributes: ["is_bot_active"],
      });
      if (!storeCheck || storeCheck.is_bot_active === false) {
        logger.warn(
          `[${storeWaId}] HARD STOP: Bot global OFF. Bubble ke-${i + 1} digugurkan.`,
        );
        return;
      }
    } catch (_) {
      /* non-critical, lanjut kirim */
    }
    ...
    const sendOptions = quotedMessageId
      ...
      : {};
    const sentMsg = await _sendActiveMessage(
      storeWaId,
      contactId,
      bubble,
      sendOptions,
    ...
    const quotedContext = isFirstBubble
      ? _quoteContextFromMessage(message, "Pelanggan")
      : {};
    await _logBotReply(
      storeWaId,
      contactId,
 ...
 */
async function _sendMediaToChat(
  message,
  mediaAsset,
  caption,
  storeWaId,
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      if (!fs.existsSync(mediaPath))
        throw new Error(`File tidak ditemukan: ${mediaAsset.filename}`);
      ...
      logger.success(
        `[${storeWaId}] Media [${mediaAsset.label}] dikirim ke [${contactId}]`,
      );
      return; // Berhasil, keluar dari retry loop
    } catch (mediaError) {
      if (attempt < 2) {
        logger.warn(
          `[${storeWaId}] Gagal kirim media (attempt ${attempt}), retry dalam 2 detik: ${mediaError.message}`,
        );
        await new Promise((r) => setTimeout(r, 2000));
      } else {
        logger.error(
          `[${storeWaId}] Gagal kirim media [${mediaAsset.label}] setelah 2x: ${mediaError.message}`,
        );
...
async function _updateConversationSummary(storeWaId, contactId, senderName) {
  try {
    ...
    const latestIdentity =
      [...history]
        .reverse()
        .find((item) => item.contact_display_name || item.sender_name) || {};
    const stableName = [
    ...
    const name =
      stableName ||
      buildContactIdentity(contactId, { phone: latestIdentity.contact_phone })
        .displayName ||
      formatWaNumber(contactId);
    ...
    // 🔧 Retry SQLite — backoff strategy untuk traffic tinggi
    let summary, created;
    ...
    logger.info(
      `[${storeWaId}] Rekap Chat [${name}${stablePhone ? " | " + stablePhone : ""}] Berhasil Diperbarui.`,
    );
    ...
    try {
      ...
      } = require("../services/smart-label.service");
      ...
      try {
        ...
        /* client mungkin sedang restart — label tetap disimpan di DB */
      }
      // Non-blocking: error di sini tidak boleh ganggu flow utama
      applyLabelsFromSummary(storeWaId, contactId, summaryText, waClient).catch(
        (e) =>
          logger.warn(
            `[${storeWaId}] Smart label non-critical error: ${e.message}`,
          ),
      );
    } catch (labelErr) {
      logger.warn(
        `[${storeWaId}] Smart label setup error: ${labelErr.message}`,
      );
    }
 ...
 */
async function _scheduleFollowUpIfNeeded(
  storeWaId,
  contactId,
  contactName,
  currentSummary,
  ...
  try {
    const { scheduleFollowUp } = require("../services/followup.service");
    ...
    const isClosedConversation = /\bstatus:\s*(closing|selesai)\b/.test(
      summaryLower,
    );

    if (isClosedConversation) {
      logger.info(
        `[FollowUp] Batal menjadwalkan untuk [${contactId}] karena status percakapan sudah closing/selesai.`,
      );
      return;
    }
...
function _triggerBackgroundSummaryIfNeeded(storeWaId, contactId, senderName) {
  ...
  const timerId = setTimeout(async () => {
    ...
    try {
      ...
      logger.warn(
        `[${storeWaId}] Background summary update gagal [${contactId}]: ${e.message}`,
      );
    }
...
async function sweepUnansweredChats(storeWaId) {
  ...
  try {
    socketService.emitSyncProgress(storeWaId, {
      status: "starting",
      message: "Menganalisis obrolan tertunda dalam 2 hari terakhir...",
    });
    ...
    if (unanswered.length === 0) {
      socketService.emitSyncProgress(storeWaId, {
        status: "completed",
        message: "Hore! Tidak ada chat tertunda yang perlu dibalas.",
      });
    ...
    for (const batchItem of unanswered) {
      try {
        ...
        logger.warn(
          `[${storeWaId}] Gagal sweep reply untuk [${msg.contact_id}]: ${e.message}`,
        );
      }
    }
    ...
    logger.success(
      `[${storeWaId}] Sweep Unanswered Chats selesai. Terbalas ${processed} kontak.`,
    );
    ...
    socketService.emitSyncProgress(storeWaId, {
      status: "error",
      message: `Gagal sapu bersih: ${err.message}`,
    });
