...
if (result.error && process.env.NODE_ENV !== "production") {
  logger.warn(
    "Peringatan: File .env tidak ditemukan (normal di production server).",
  );
}
...
const config = {
  ...
  // Default: deepseek-chat (model resmi DeepSeek V3 API)
  // 'deepseek-v4-pro' bukan nama model valid di DeepSeek API — gunakan 'deepseek-chat' atau 'deepseek-reasoner'
...
const validateConfig = () => {
  if (!config.OPENAI_API_KEY && !process.env.DEEPSEEK_API_KEY) {
    logger.error(
      "CRITICAL: Harus menyediakan setidaknya OPENAI_API_KEY atau DEEPSEEK_API_KEY di .env!",
    );
    return false;
  }
  return true;
