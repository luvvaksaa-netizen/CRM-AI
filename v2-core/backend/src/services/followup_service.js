...
const { FollowUp, Store, ChatSummary, MediaAsset, BotAgent } = require('../models/index');
...
async function executeFollowUp(followUp) {
    ...
    const { ChatMessage, ChatSummary: ChatSummaryModel } = require('../models/index');
    ...
    while (true) {
        try {
            ...
            const isRestartError = /null|evaluate|detached|not ready|belum siap|getChat|undefined|timeout|closed|destroyed|protocol error/i.test(sendErr.message);
