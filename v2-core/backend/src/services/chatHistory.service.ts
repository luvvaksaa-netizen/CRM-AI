...
import { mergeStableContactIdentity, firstStableDisplayName } from './contactIdentity.service';
import logger from '../utils/logger';

export function clipQuotedBody(value: any, maxLength: number = 700): string | null {
 ...
 */
async function withSqliteRetry<T>(fn: () => Promise<T>, maxRetries = 5): Promise<T> {
  let lastErr: any;
  for (let i = 0; i < maxRetries; i++) {
    try {
      return await fn();
    } catch (err: any) {
      const isBusy = err?.message?.includes('SQLITE_BUSY') || err?.parent?.message?.includes('SQLITE_BUSY');
      if (!isBusy) throw err;
      lastErr = err;
...
export async function addToChatHistory(storeId: string, msg: any): Promise<void> {
  try {
    await withSqliteRetry(async () => {
      // ═══ DEDUP GUARD ═══
      const waMessageId: string | null = msg.wa_message_id || msg.id || null;
      if (waMessageId) {
        const existing = await (ChatMessage as any).findOne({ where: { wa_message_id: waMessageId } });
        if (existing) return;
      }
      ...
      const quotedMessageId: string | null = msg.quoted_message_id || msg.quotedMessageId || null;
      let quotedRecord: any = null;
      if (quotedMessageId) {
        quotedRecord = await (ChatMessage as any).findOne({
