...
/**
 * Sanitize malformed JSON strings untuk parsing yang lebih robust.
 * Menangani:
 * - Control characters (\x00-\x1F, \x7F)
 * - Trailing commas dalam objects dan arrays
 * - Single quotes untuk property names dan values
...
function sanitizeJSON(input) {
  ...
  let sanitized = input
    ...
    // '"key"' => '"key"' (if already has quotes after conversion)
    // 'key' => '"key"' (unquoted to quoted)
    .replace(/'([^']*)'\s*:/g, '"$1":')
    ...
    // Hanya untuk pattern yang belum di-quote: word: value
...
/**
 * Parse JSON dengan robust fallbacks dan sanitization.
 * @param {string} content - JSON string to parse
 * @returns {object} Parsed object atau empty object jika semua fallback gagal
 */
function parseJSONWithFallback(content) {
  ...
  try {
    ...
    if (sanitized !== contentText) {
      try {
        ...
        logger.warn(
          "[Learning] Fallback 3 (sanitasi) berhasil. Sanitized: " +
      ...
      try {
        ...
        if (matchSanitized) {
          ...
          logger.warn(
            "[Learning] Fallback 3b (sanitasi + extract object) berhasil",
          );
          return parsed;
        }
      } catch (e5) {
      ...
      try {
        ...
        if (matchSanitized) {
          parsed = JSON.parse(matchSanitized[0]);
          logger.warn(
            "[Learning] Fallback 3c (sanitasi + extract array) berhasil",
          );
          return parsed;
        }
      } catch (e6) {
  ...
  logger.error(
    "[Learning] Semua JSON parse fallback gagal. Sample content:",
...
function detectProductType(chatText) {
  ...
  if (/\buv\b|stiker keras|timbul|botol|helm|tumbler|kaca|ember/.test(lower))
    return "uv";
  if (/\bdtf\b|setrika|baju|kain|seragam|hijab|label nama/.test(lower))
    return "dtf";
  return "generic";
}
...
function calculateQualityScore(chatText, analysisData = {}) {
  ...
  const hasFlowComplete = /rekap|nota|total.*bayar|pembayaran ke/i.test(
    chatText,
  );
  ...
  const hasConfirmation =
    /iya.*sesuai|sudah sesuai|benar|iya.*betul|ok.*(jalan|kirim)|siap/i.test(
      chatText,
    );
  ...
  const hasCompleteData =
    /nama penerima/i.test(chatText) && /kecamatan|kec\s*:/i.test(chatText);
  ...
  const hasRepeatQuestion =
    /(nama.*cetak|nama.*label|mau pesan|berapa paket).{0,50}(nama.*cetak|nama.*label|mau pesan|berapa paket)/is.test(
      chatText,
    );
  if (hasRepeatQuestion) score -= 1.5;

  // Negatif: ada komplain serius
  const hasSeriousComplaint =
    /ketipu|tidak jadi|cancel|batal|tidak sampai|hilang/i.test(chatText);
  if (hasSeriousComplaint) score -= 2;

  // Negatif: percakapan sangat panjang (>100 pesan = ada masalah)
  const messageCount = (chatText.match(/^\d{2}\/\d{2}\/\d{2}/gm) || []).length;
...
async function onManualChatIdle(storeWaId, contactId, agentId) {
  ...
  const timer = setTimeout(
    ...
    5 * 60 * 1000,
  ); // 5 menit debounce (tunggu chat benar-benar idle)
...
async function saveOrUpdatePattern(pattern, meta = {}) {
  ...
  try {
    ...
    if (existing) {
      ...
      if (
        contoh_kalimat &&
        contoh_kalimat.length > (existing.contoh_kalimat || "").length
      ) {
        existing.contoh_kalimat = contoh_kalimat;
      }
      await existing.save();
      logger.info(
        `[Learning] ✅ Pattern "${teknik}" diperbarui (freq: ${existing.frequency}, conf: ${existing.confidence.toFixed(2)})`,
      );
      return existing;
    } else {
      // Buat baru
      ...
      logger.info(
        `[Learning] ✨ Pattern baru "${teknik}" tersimpan (agent: ${agentId})`,
      );
      return newPattern;
    }
  } catch (err) {
...
async function saveAnalytic(data) {
  ...
  try {
    await ClosingAnalytic.create({
      ...
      closing_probability:
        data.closingProbability !== undefined ? data.closingProbability : null,
...
async function onClosingDetected(storeWaId, contactId, agentId = null) {
  ...
  if (_closingDebounce.has(debounceKey)) {
    logger.info(
      `[Learning] [${contactId}] Debounce aktif — skip analisis ganda.`,
    );
    return;
  }
  ...
  _runClosingAnalysis({
    ...
    logger.warn(`[Learning] Background analysis error: ${e.message}`),
  );
}
 ...
 * @param {object} opts
 */
async function _runClosingAnalysis({
  storeWaId,
  contactId,
  agentId,
  chatText,
  ...
  try {
    ...
    if (!fullChatText && storeWaId && contactId) {
      ...
      if (messages.length < 5) {
        logger.info(
          `[Learning] Percakapan [${contactId}] terlalu pendek (${messages.length} msg). Skip.`,
        );
        return;
      }
      // Format natural — agar AI mudah mengenali pola percakapan dan menghitung pesan
      fullChatText = messages
        .map((m) => {
    ...
    logger.info(
      `[Learning] Menganalisis percakapan [${contactId || sourceFile}], Tipe: ${productType}`,
    );
    ...
    if (
      analysis &&
      analysis.closing_probability !== undefined &&
      storeWaId &&
      contactId
    ) {
      try {
    ...
    const messageCount =
      analysis.jumlah_pesan || countMessagesUntilClosing(fullChatText);
    const metodeBayar =
      analysis.metode_bayar || detectPaymentMethod(fullChatText);

    logger.info(
      `[Learning] 📊 Score: ${finalScore}/10, Pesan: ${messageCount}, Pola: ${patterns.length}`,
    );
    ...
    if (patterns.length === 0) {
      logger.info(
        `[Learning] Tidak ada pola yang diekstrak untuk [${contactId || sourceFile || "dataset"}].`,
      );
      return;
    }

    logger.info(
      `[Learning] ✅ Quality Gate LULUS (Score: ${finalScore}/10) — mulai simpan ${patterns.length} pola.`,
    );
    ...
    logger.info(
      `[Learning] ✅ Analisis selesai: ${savedCount} pola tersimpan (score: ${finalScore}/10, prob: ${analysis.closing_probability || "?"}%)`,
    );
  } catch (err) {
...
async function processDatasetFile(filePath, agentId = null) {
  try {
    ...
    logger.info(
      `[Learning] 📂 Memproses dataset: ${fileName} (${chatText.length} chars)`,
    );
    ...
    logger.warn(
      `[Learning] Gagal proses dataset ${path.basename(filePath)}: ${err.message}`,
    );
...
async function getLearningStats(agentId = null) {
  ...
  try {
    ...
    const totalPatterns = await ClosingPattern.count({
    ...
    const avgScore = await ClosingAnalytic.findOne({
      attributes: [
        [sequelize.fn("AVG", sequelize.col("conversation_score")), "avg_score"],
      ],
      where: whereAgent,
      raw: true,
    });
    ...
    const avgScoreLast7 =
      recentAnalytics.length > 0
        ? recentAnalytics.reduce((s, a) => s + (a.conversation_score || 0), 0) /
    ...
    const datasetCount = await ClosingAnalytic.count({
      where: { ...whereAgent, source_type: "dataset" },
    });
    const productionCount = await ClosingAnalytic.count({
      where: { ...whereAgent, source_type: "production" },
    });
  ...
  if (agentId) {
    try {
      const agent = await BotAgent.findByPk(agentId, {
        attributes: ["learned_prompt_addon"],
      });
      if (
        agent?.learned_prompt_addon &&
        agent.learned_prompt_addon.trim().length > 50
      ) {
      ...
      /* Jika gagal baca, fallback ke raw patterns */
    }
  }
...
async function getRecentPromptRefinements(agentId, limit = 3) {
  ...
  try {
    ...
    for (const a of analytics) {
      ...
      try {
        const analysis =
          typeof a.analysis_json === "string"
            ? JSON.parse(a.analysis_json)
            : a.analysis_json || {};
        if (
          analysis.rekomendasi_prompt_ai &&
          analysis.rekomendasi_prompt_ai.tambah_aturan
        ) {
