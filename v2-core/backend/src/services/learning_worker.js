...
const IDLE_DEBOUNCE_MS = 20 * 60 * 1000; // 20 menit idle sebelum analisis (hemat token)
const DEDUP_WINDOW_MS = 60 * 60 * 1000; // Kontak yang sama tidak dianalisis 2x dalam 1 jam
const REVISION_EVERY_N_JOBS = 20; // Setiap 20 analisis → revisi prompt (bukan 3!)
const MAX_PATTERNS_INJECT = 8; // Maks pola yang dibaca saat revisi
const MAX_QUEUE_SIZE = 50; // Batas atas antrian lebih ketat
...
function pushLearningJob(job) {
  ...
  const timer = setTimeout(() => {
    ...
    if (_queue.length >= MAX_QUEUE_SIZE) {
      logger.warn(
        "[LearningWorker] Queue penuh, job dibuang untuk mencegah memory leak.",
      );
      return;
    }

    // Cek duplikat dalam antrian
    const alreadyQueued = _queue.some(
      (q) => q.storeWaId === job.storeWaId && q.contactId === job.contactId,
    );
    if (!alreadyQueued) {
...
async function _processNextJob() {
  ...
  try {
    logger.info(
      `[LearningWorker] 🔄 Memproses job: [${job.storeWaId}] kontak [${_formatId(job.contactId)}]`,
    );
    ...
    logger.info(
      `[LearningWorker] ✅ Analisis selesai untuk [${job.contactId}]`,
    );
    ...
    logger.warn(
      `[LearningWorker] ❌ Error memproses job [${key}]: ${err.message}`,
    );
  } finally {
    _isRunning = false;
  }
...
async function _reviseAgentPrompt(agentId) {
  ...
  logger.info(
    `[LearningWorker] 🧠 Memulai revisi prompt untuk agent [${agentId || "global"}]`,
  );
  ...
  try {
    ...
    if (!agent && agentId) {
      logger.warn(
        `[LearningWorker] Agent [${agentId}] tidak ditemukan, skip revisi.`,
      );
      return;
    }
    ...
    if (topPatterns.length === 0) {
      logger.info(
        `[LearningWorker] Tidak ada pola untuk digunakan dalam revisi agent [${agentId}].`,
      );
      return;
    }
...
Berikan HANYA objek JSON berikut tanpa komentar lain:
{
  ...
  "ringkasan_perubahan": "<1-3 kalimat: apa yang ditambah, apa yang dihapus, kenapa>"
    ...
    try {
      ...
      logger.warn(
        `[LearningWorker] Gagal parse hasil revisi JSON: ${e.message}`,
      );
      return;
    }
    ...
    logger.info(
      `[LearningWorker] ✨ Revisi prompt selesai untuk agent [${agentId || "global"}]. Perubahan: ${summaryText}`,
    );
  } catch (err) {
    logger.warn(
      `[LearningWorker] Error revisi prompt agent [${agentId}]: ${err.message}`,
    );
  }
}
...
function startWorker() {
  ...
  _workerTimer = setInterval(async () => {
    ...
    try {
      ...
      logger.warn(
        `[LearningWorker] Uncaught error in worker tick: ${e.message}`,
      );
    }
  }, WORKER_TICK_MS);
  logger.info(
    "[LearningWorker] 🚀 Background learning worker aktif (interval 5 detik).",
  );
}
