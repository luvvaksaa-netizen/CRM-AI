...
async function injectWajs(client, storeWaId) {
  ...
  if (!client?.pupPage) {
    logger.warn(
      `[${storeWaId}] WA-JS tidak bisa diinjeksi: Puppeteer page belum tersedia.`,
    );
    return false;
  }
  ...
  if (!bundlePath) {
    logger.warn(
      `[${storeWaId}] Paket @wppconnect/wa-js belum terpasang. Lanjut memakai WWebJS saja.`,
    );
  ...
  try {
    try {
      ...
      await client.pupPage.waitForFunction(() => Boolean(window.WPP), {
        timeout: WAJS_TIMEOUT_MS,
      });
    } catch (scriptTagError) {
      logger.warn(
        `[${storeWaId}] WA-JS addScriptTag belum berhasil (${scriptTagError.message}). Mencoba inline injection...`,
      );
    ...
    const version = await client.pupPage
      .evaluate(() => window.WPP?.version || "unknown")
      .catch(() => "unknown");
    client.__wajsReady = true;
    logger.success(
      `[${storeWaId}] WA-JS aktif (WPP ready, version: ${version}).`,
    );
    return true;
  } catch (error) {
    client.__wajsReady = false;
    logger.warn(
      `[${storeWaId}] WA-JS gagal diinjeksi: ${error.message}. Sistem tetap memakai WWebJS.`,
    );
    return false;
  }
}
...
function normalizeWidPayload(value) {
  ...
  if (typeof value === "string")
    return { serialized: value, user: value.split("@")[0] };
  const serialized =
    value._serialized ||
    value.serialized ||
  ...
  const user =
    value.user ||
    value.id?.user ||
    String(serialized || "").split("@")[0] ||
    "";
  const server =
    value.server ||
    value.id?.server ||
    String(serialized || "").split("@")[1] ||
    "";
...
async function sendReactionToMessage(
  message,
  emoji = "\uD83D\uDC4D",
  storeWaId = "default",
) {
  ...
  return page.evaluate(async (id) => {
    ...
    const cleanWid = (wid) => {
      ...
      if (typeof wid === "string")
        return { serialized: wid, user: wid.split("@")[0] };
      const serialized =
        wid._serialized ||
        wid.serialized ||
        wid.id?._serialized ||
...
async function markIsComposing(
  client,
  chatId,
  duration = 6000,
  storeWaId = "default",
...
async function safeMarkIsComposing(
  client,
  chatId,
  duration,
  storeWaId = "default",
...
async function editLabel(client, labelId, updates = {}, storeWaId = "default") {
  ...
  return page.evaluate(
    ...
    },
    {
      ...
      labelColor:
        typeof updates.color === "number"
          ? updates.color
          : String(updates.color || "").trim() || undefined,
    },
  );
}
...
async function getChatLabels(client, chatId, storeWaId = "default") {
  ...
  return page.evaluate(async (id) => {
    ...
    const chat = window.WPP?.chat?.get?.(id) || window.Store?.Chat?.get?.(id);
    if (!chat) return [];
...
async function addOrRemoveLabels(
  client,
  chatIds,
  labelOps,
  storeWaId = "default",
  ...
  if (cleanIds.length === 0)
    throw new Error("Minimal satu contactId wajib diisi.");

  const options = normalizeLabelOptions(labelOps);
  const page = await getReadyPage(client, storeWaId);
  return page.evaluate(
    ({ targetChatIds, labelOptions }) => {
...
async function ensureLabel(client, name, color, storeWaId = "default") {
  ...
  // Case-insensitive match — cegah duplikat akibat perbedaan kapital
  const existing = labels.find(
    (label) =>
      String(label.name || "").toLowerCase() === cleanName.toLowerCase(),
  );
  if (existing) return existing;

  // Guard limit 20 label WhatsApp Business
  // WA Business memiliki batas keras 20 label per akun.
  // Jika sudah 20, createLabel akan gagal silent di WPP — lebih baik throw lebih awal.
  const WA_LABEL_LIMIT = 20;
  if (labels.length >= WA_LABEL_LIMIT) {
    const logger = require("../utils/logger");
    logger.warn(
      `[${storeWaId}] Batas maksimal ${WA_LABEL_LIMIT} label WA sudah tercapai (${labels.length} label). Label "${cleanName}" tidak dibuat. Hapus label lama dulu.`,
    );
    throw new Error(
      `Batas maksimal ${WA_LABEL_LIMIT} label WhatsApp tercapai. Hapus label yang tidak dipakai dulu.`,
    );
  }
...
async function safeAddLabelByName(
  client,
  chatIds,
  name,
  color,
...
async function sendReactionById(
  client,
  messageId,
  emoji,
  storeWaId = "default",
...
async function forwardMessages(
  ...
  const page = await getReadyPage(client, storeWaId);
  return page.evaluate(
    ({ to, idsToForward, forwardOptions }) => {
      if (window.WPP?.chat?.forwardMessages) {
        return window.WPP.chat.forwardMessages(
          to,
          idsToForward,
          forwardOptions || {},
        );
      ...
      if (window.WPP?.chat?.forwardMessage && idsToForward.length === 1) {
        return window.WPP.chat.forwardMessage(
          to,
          idsToForward[0],
          forwardOptions || {},
        );
...
async function getChats(client, storeWaId = "default") {
  ...
  const rawChats = await page.evaluate(async () => {
    ...
    return (chats || []).map((chat) => ({
      ...
      name:
        safeGet(() => chat.name) || safeGet(() => chat.formattedTitle) || "",
  ...
  return rawChats.map((chat) => ({
    ...chat,
    sendSeen: async () =>
      safeMarkIsRead(client, chat.id._serialized, storeWaId),
  }));
}
  ...
  const rawMessages = await page.evaluate(
    async ({ id, count }) => {
      ...
      const getMsgId = (idObj) => {
        ...
        if (idObj.id?._serialized) return idObj.id._serialized;
        if (idObj.remote && idObj.id)
          return `${idObj.fromMe ? "true" : "false"}_${getWaId(idObj.remote)}_${idObj.id}`;
        return String(idObj);
      };
      const safeGet = (fn, fallback = null) => {
        try {
      ...
      return (msgs || []).map((msg) => ({
        ...(() => {
          const quoted =
            safeGet(() => msg.quotedMsg, null) ||
            safeGet(
              () =>
                typeof msg.quotedMsgObj === "function"
                  ? msg.quotedMsgObj()
                  : null,
              null,
            );
          ...
          return {
            ...
            quotedBody:
              safeGet(() => quoted?.body, "") ||
              safeGet(() => quoted?.caption, "") ||
              "",
            quotedFromMe: Boolean(
              safeGet(() => quoted?.id?.fromMe, false) ||
              safeGet(() => quoted?.fromMe, false),
            ),
        ...
        fromMe: Boolean(
          safeGet(() => msg.id?.fromMe, false) ||
          safeGet(() => msg.fromMe, false),
        ),
        timestamp:
          safeGet(() => msg.timestamp, null) ||
          safeGet(() => msg.t, null) ||
          Math.floor(Date.now() / 1000),
        hasMedia: Boolean(
          safeGet(() => msg.hasMedia, false) ||
          safeGet(() => msg.isMedia, false) ||
          safeGet(() => msg.mediaData, null),
        ),
