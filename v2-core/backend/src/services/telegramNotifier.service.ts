...
async function getTelegramConfig(): Promise<{ token: string; chatId: string; enabled: boolean }> {
  const { AppConfig } = await import('../models');
  const tokenRec = await AppConfig.findOne({ where: { key: 'openai_billing_telegram_token' } });
  const chatRec = await AppConfig.findOne({ where: { key: 'openai_billing_telegram_chat_id' } });
  const enabledRec = await AppConfig.findOne({ where: { key: 'openai_billing_telegram_enabled' } });
  return {
...
export async function sendDailyBillingReport(usage: number, balance: number, date: string): Promise<boolean> {
...
export async function sendThresholdAlert(usage: number, threshold: number): Promise<boolean> {
