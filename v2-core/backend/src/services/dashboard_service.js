...
const { Store, ChatMessage, sequelize } = require('../models/index');
...
const { addToChatHistory: _addToChatHistory } = require('./chatHistory.service');
const { mergeStableContactIdentity: _mergeStableContactIdentity, updateContactPhoneIdentity: _updateContactPhoneIdentity, isPlaceholderContactName: _isPlaceholderContactName, firstStableDisplayName: _firstStableDisplayName, clipQuotedBody: _clipQuotedBody } = require('./contactIdentity.service');
...
/**
 * Set the Socket.IO instance for v2-core.
 * Called from app.ts after server is created so that
 * addToChatHistory and other emitters can broadcast events.
 */
...
const isPlaceholderContactName = _isPlaceholderContactName;

const firstStableDisplayName = _firstStableDisplayName;

const mergeStableContactIdentity = _mergeStableContactIdentity;

const clipQuotedBody = _clipQuotedBody;
...
function initDashboard(port = 3000) {
  ...
  try {
    const { initFollowUpScheduler } = require('./followup.service');
  ...
  app.delete('/api/agents/:id', authorize('admin'), async (req, res) => {
    try {
      ...
      const { BotAgent, MediaAsset } = require('../models/index');
  ...
  app.get('/api/stores', async (req, res) => {
    try {
      ...
      res.json(stores.map(s => ({ ...s.dataValues, status: storeStatuses[s.wa_id] || 'disconnected' })));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });

  // GET: Status koneksi per wa_id
  app.get('/api/stores/status', (req, res) => {
  ...
  app.get('/api/summaries', async (req, res) => {
    try {
      const { ChatSummary, Store } = require('../models/index');
  ...
  app.get('/api/analytics/overview', async (req, res) => {
    try {
      const { ChatSummary, ChatMessage, Store } = require('../models/index');
  ...
  app.get('/api/analytics/followups', async (req, res) => {
    try {
      const { FollowUp, Store } = require('../models/index');
  ...
  app.get('/api/followups/stats/:storeId', async (req, res) => {
    try {
      const { getFollowUpStats } = require('./followup.service');
  ...
  app.get('/api/followups/:storeId', async (req, res) => {
    try {
      const { getFollowUps } = require('./followup.service');
  ...
  app.post('/api/followups/cancel/:id', authorize('operator'), async (req, res) => {
    try {
      const { cancelFollowUpById } = require('./followup.service');
  ...
  app.post('/api/followups/emergency-cancel-all', async (req, res) => {
    try {
      const { FollowUp } = require('../models/index');
  ...
  app.get('/api/stores/:id/followup-config', authorize('operator'), async (req, res) => {
    try {
      const { getFollowUpConfig } = require('./followup.service');
  ...
  app.delete('/api/stores/:id', authorize('admin'), async (req, res) => {
    try {
      const wa_id = req.params.id;
      const whatsappService = require('../whatsapp_service');
      const { ChatMessage, ChatSummary } = require('../models/index');
  ...
  app.post('/api/send-media', manualSendLimiter, authorize('operator'), async (req, res) => {
    try {
      ...
      const { MediaAsset, Store } = require('../models/index');
...
const addToChatHistory = _addToChatHistory;

const updateContactPhoneIdentity = _updateContactPhoneIdentity;

function emitTypingStatus(storeId, contactId, isTyping) {
  const { socketService } = require('./socket.service');
  socketService.emitTypingStatus(storeId, contactId, isTyping);
}

function emitQRSpec(storeId, qr) {
  const { socketService } = require('./socket.service');
  socketService.emitQRUpdate(storeId, qr);
}
...
function emitMessageRevoked(storeId, waMessageId, contactId) {
  const { socketService } = require('./socket.service');
  socketService.emitMessageRevoked(storeId, waMessageId, contactId);
}
