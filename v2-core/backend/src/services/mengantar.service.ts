...
// ── Search Address ─────────────────────────────────────────────────────────────
export async function searchAddress(keyword: string): Promise<MengantarAddressSearchResult | null> {
...
export async function getShippingCost(destinationCity: string, weightGrams: number = 1000): Promise<string> {
    try {
...
// ── Create Order (Resi) ───────────────────────────────────────────────────────
export async function createOrder(params: MengantarCreateOrderParams): Promise<MengantarCreateOrderResult> {
    const client = _getAuthClient();
    if (!client) {
    ...
    const {
        ...
        weight = 1, quantity = 1, parcelContent,
        goodsValue, codAmount, courier, addressId,
        customProducts, pickupType = 'dropOff', deliveryInstruction,
    } = params;
    ...
    try {
        ...
        if (!destId && destinationKeyword) {
            ...
            if (!destData) {
                return { success: false, error: `Wilayah "${destinationKeyword}" tidak ditemukan` };
            }
            destId = destData.id;
        ...
        if (pickupType === 'scheduledPickup') {
            ...
            if (futureTime) {
                pickup = { type: 'scheduledPickup', volume: 'volumeMotor', address_id: pickupAddressId, time_id: futureTime._id };
            } else {
        ...
        console.log(`[Mengantar] ✅ Order: cnote=${cnoteNo} order_id=${orderId} batch=${batch} paid=${isPaid}`);
...
// ── Format Resi Message ───────────────────────────────────────────────────────
export function formatResiMessage(orderResult: MengantarCreateOrderResult): string {
    ...
    if (orderResult.is_unpaid) {
        lines.push(`⚠️ Catatan: Resi masih perlu diaktivasi (saldo pengiriman sedang diproses).`);
        lines.push(``);
    }

    if (orderResult.cnote_no) {
        lines.push(`Bisa lacak paket di aplikasi ${orderResult.courier || 'J&T'} ya bund, atau cek di Mengantar Pelacakan 😊`);
    }
