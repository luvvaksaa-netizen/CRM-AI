...
import {
  MessageSquare, Phone, Search, MoreVertical, Bot, User, Check, CheckCheck,
  RefreshCw, Image as ImageIcon, Video, X, Reply, Paperclip, ArrowLeft, Pause, Play, Tag,
  Copy, Trash2, FileText, RotateCw, Smile, Share2,
} from 'lucide-react';
...
const parseMessageBody = (body: string) => {
  ...
  if (mediaMatch) {
    ...
    return { type: isVideo ? 'video' as const : 'image' as const, url: `${UPLOAD_BASE}${path}`, caption };
  }
...
const MessageBody = ({ body }: { body: string }) => {
  ...
  if (parsed.type === 'text') {
    return <span className="whitespace-pre-wrap">{parsed.text || '(Media/Lampiran)'}</span>;
  }
  return (
    <div className="space-y-2">
      {parsed.type === 'image' ? (
        <img src={parsed.url} alt="Media" className="max-w-full rounded-lg max-h-64 object-cover" loading="lazy" />
      ) : (
        <video src={parsed.url} controls className="max-w-full rounded-lg max-h-64" />
      )}
      {parsed.caption && <p className="text-sm whitespace-pre-wrap">{parsed.caption}</p>}
    </div>
  );
};
...
const QuotedBlock = ({ msg }: { msg: ChatMessage }) => {
  ...
  return (
    <div className="mb-1.5 px-2 py-1.5 rounded-lg border-l-2 border-blue-400 bg-black/20 text-xs opacity-80">
      <span className="font-semibold block">{msg.quoted_sender_name || (msg.quoted_from_me ? 'Anda' : 'Customer')}</span>
      <span className="line-clamp-2">{formatPreview(msg.quoted_body || '')}</span>
    </div>
  );
};
...
const ChatManagement = () => {
  ...
  const fetchMediaAssets = useCallback(async () => {
    ...
    try {
      const res = await api.get('/media', { params: { agent_id: store.agent_id } });
  ...
  const handleToggleBot = async () => {
    ...
    try {
      if (activeContact.is_bot_paused) {
        await api.post(`/chat/${selectedStore}/${activeContact.contact_id}/unpause`);
        updateContactPauseState(activeContact.contact_id, false, null);
        toast.success('AI diaktifkan kembali untuk kontak ini');
      } else {
        await api.post(`/chat/${selectedStore}/${activeContact.contact_id}/pause`, { durationMinutes: 30 });
  ...
  const handleRequestPhone = async () => {
    ...
    try {
      const res = await api.post(`/chat/${selectedStore}/${encodeURIComponent(activeContact.contact_id)}/request-phone`);
      if (res.data.phone) {
        setResolvedPhone(res.data.phone);
        toast.success(`Nomor: +${res.data.phone}`);
      } else {
        toast.success(res.data.message || 'Permintaan nomor dikirim ke WhatsApp');
      }
    } catch (err: any) {
  ...
  const openLabelModal = async () => {
    ...
    try {
      const [waRes, contactRes] = await Promise.all([
        api.get(`/smart-labels/${selectedStore}/wa-list`),
        api.get(`/smart-labels/${selectedStore}/${encodeURIComponent(activeContact.contact_id)}`),
      ]);
  ...
  const handleSaveLabels = async () => {
    ...
    try {
      const res = await api.post(
        `/smart-labels/${selectedStore}/${encodeURIComponent(activeContact.contact_id)}/update`,
        { add, remove },
      );
  ...
  const handleSyncLabelsFromWa = async () => {
    ...
    try {
      const res = await api.post(
        `/smart-labels/${selectedStore}/${encodeURIComponent(activeContact.contact_id)}/sync`,
      );
      const labels = res.data.labels || [];
      setEditingLabels([...labels]);
      setOriginalLabels([...labels]);
  ...
  const handleCreateLabel = async () => {
    ...
    setLoadingLabels(true);
    try {
      await api.post('/smart-label/' + selectedStore + '/create', { name: newLabelName.trim(), color: newLabelColor });
  ...
  const handleSendReaction = async (waMessageId: string, emoji: string) => {
    ...
    setShowReactionPicker(null);
    try {
      await api.post(`/chat/${selectedStore}/messages/reaction`, { messageId: waMessageId, emoji });
  ...
  const handleClearChat = async () => {
    ...
    setShowContactMenu(false);
    try {
      await api.delete(`/chat/${selectedStore}/${encodeURIComponent(activeContact.contact_id)}`);
      setMessages([]);
      setActiveContact(null);
      setShowMobileChat(false);
      await fetchContacts();
      ...
      toast.error(err.response?.data?.message || 'Gagal menghapus chat (perlu role admin)');
    }
  };
  ...
  const filteredContacts = contacts.filter(c => {
    ...
    if (chatFilter === 'label' && selectedLabel && !(c.labels || []).includes(selectedLabel)) return false;
    return true;
  });
  ...
  const renderMessage = (msg: ChatMessage) => {
        ...
        >
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 ml-1">
              ...
              {canReply && (
                <button onClick={onReply} className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-blue-400 transition-all" title="Balas">
                  <Reply className="w-3 h-3" />
                </button>
              )}
              {canReply && (
                <div className="relative">
                  <button onClick={() => setShowReactionPicker(showReactionPicker === msg.wa_message_id ? null : (msg.wa_message_id || null))} className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-yellow-400 transition-all" title="Reaksi">
                    <Smile className="w-3 h-3" />
                  </button>
                  {showReactionPicker === msg.wa_message_id && (
                    <div data-reaction-picker className="absolute bottom-full left-0 mb-1 flex gap-1 bg-slate-800 border border-slate-600 rounded-xl p-1.5 z-20 shadow-xl">
                      {['👍','❤️','😂','😮','😢','🙏'].map(emoji => (
                        <button key={emoji} onClick={() => handleSendReaction(msg.wa_message_id!, emoji)} className="hover:scale-125 transition-transform text-sm">{emoji}</button>
                      ))}
                    </div>
                  )}
                </div>
              ...
              {canReply && (
                <button onClick={() => { setShowForwardPicker(true); setForwardMsgId(msg.wa_message_id || null); setForwardTarget(''); }} className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-emerald-400 transition-all" title="Forward">
    ...
    return (
      ...
      >
        <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 border ${msg.sender_name === 'CS Manual' ? 'bg-purple-500/20 text-purple-400 border-purple-500/30' : 'bg-blue-500/20 text-blue-400 border-blue-500/30'}`}>
          {msg.sender_name === 'CS Manual' ? <User className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
        </div>
        <div>
          <div
          ...
          <div className="flex items-center justify-end gap-1 mt-1 mr-1">
            {canReply && (
              <button onClick={onReply} className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-blue-400 transition-all mr-1" title="Balas">
                <Reply className="w-3 h-3" />
              </button>
            )}
            {canReply && (
              <div className="relative mr-1">
                ...
                {showReactionPicker === msg.wa_message_id && (
                  <div data-reaction-picker className="absolute bottom-full right-0 mb-1 flex gap-1 bg-slate-800 border border-slate-600 rounded-xl p-1.5 z-20 shadow-xl">
                    {['👍','❤️','😂','😮','😢','🙏'].map(emoji => (
                      <button key={emoji} onClick={() => handleSendReaction(msg.wa_message_id!, emoji)} className="hover:scale-125 transition-transform text-sm">{emoji}</button>
                    ))}
                  </div>
            ...
            {canReply && (
              <button onClick={() => { setShowForwardPicker(true); setForwardMsgId(msg.wa_message_id || null); setForwardTarget(''); }} className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-emerald-400 transition-all mr-1" title="Forward">
                <Share2 className="w-3 h-3" />
              </button>
            )}
            <span className="text-[10px] text-slate-500">{formatTime(msg.timestamp)} • {msg.sender_name}</span>
            {msg.is_read ? <CheckCheck className="w-3 h-3 text-blue-400" /> : <Check className="w-3 h-3 text-slate-500" />}
          </div>
        </div>
      </motion.div>
    );
  ...
  return (
    <div className="h-full flex flex-col -mx-4 lg:-mx-8 -my-4 lg:-my-8 relative overflow-hidden bg-[#09090b] dark:bg-white">
      ...
      <div className="flex h-full relative z-10">
        {/* Sidebar Contacts */}
        <div className={`${showMobileChat ? 'hidden md:flex' : 'flex'} w-full md:w-80 lg:w-96 flex-col bg-slate-900/40 dark:bg-white border-r border-slate-800/50 dark:border-slate-200 backdrop-blur-xl shrink-0 h-[calc(100vh-64px)] lg:h-screen`}>
          <div className="p-4 border-b border-slate-800/50 dark:border-slate-200 space-y-3">
            ...
            <div className="flex gap-1 overflow-x-auto custom-scrollbar pb-0.5">
              {([
                ...
                { id: 'unread' as ChatFilter, label: `Belum dibaca${unreadTotal > 0 ? ` (${unreadTotal})` : ''}` },
                { id: 'label' as ChatFilter, label: 'Label' },
              ]).map(tab => (
                <button
                  key={tab.id}
                  onClick={() => { setChatFilter(tab.id); if (tab.id !== 'label') setSelectedLabel(''); }}
            ...
            {chatFilter === 'label' && (
              <div className="flex gap-1 flex-wrap">
                {availableLabels.length === 0 ? (
                  <span className="text-xs text-slate-500">Belum ada label</span>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
            {loadingContacts ? (
              ...
              <p className="text-center text-sm text-slate-500 py-8">
                {chatFilter === 'unread' ? 'Tidak ada chat belum dibaca' : chatFilter === 'label' && selectedLabel ? `Tidak ada kontak dengan label "${selectedLabel}"` : 'Tidak ada kontak'}
              </p>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-0.5">
                      <h4 className={`text-sm font-semibold truncate ${chat.unread_count > 0 ? 'text-white' : 'text-slate-300'}`}>{displayName}</h4>
                      <span className={`text-xs shrink-0 ml-2 ${chat.unread_count > 0 ? 'text-blue-400 font-medium' : 'text-slate-500'}`}>
                        {chat.last_seen ? formatTime(chat.last_seen) : ''}
                      </span>
                    </div>
                    <p className={`text-xs truncate ${chat.unread_count > 0 ? 'text-slate-300 font-medium' : 'text-slate-500'}`}>
                      {formatPreview(chat.last_message)}
                    </p>
                    {(chat.labels || []).length > 0 && (
                      <div className="flex gap-1 mt-1 flex-wrap">
                        {(chat.labels || []).slice(0, 2).map(l => (
                          <span key={l} className="text-[9px] px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">{l}</span>
                        ))}
                      </div>
                    )}
                  </div>
        ...
        {/* Chat Area */}
        <div className={`${showMobileChat ? 'flex' : 'hidden md:flex'} flex-1 flex-col h-[calc(100vh-64px)] lg:h-screen`}>
          {activeContact ? (
            <>
              {/* Header */}
              <div className="h-16 px-4 md:px-6 border-b border-slate-800/50 dark:border-slate-200 bg-slate-900/40 dark:bg-white backdrop-blur-xl flex justify-between items-center shrink-0">
                <div className="flex items-center gap-3">
                  <button onClick={() => setShowMobileChat(false)} className="md:hidden p-1.5 rounded-lg hover:bg-slate-800 text-slate-400">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium flex items-center gap-1 mt-0.5">
                      {activeContact.is_bot_paused ? (
                        <><span className="w-1.5 h-1.5 rounded-full bg-amber-500" /><span className="text-amber-400">AI Paused</span></>
                      ) : (
                        <><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /><span className="text-emerald-400">AI Active</span></>
                      )}
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    title={activeContact.is_bot_paused ? 'Aktifkan AI' : 'Pause AI 30 menit'}
                  ...
                  >
                    {activeContact.is_bot_paused ? <Play className="w-3.5 h-3.5" /> : <Pause className="w-3.5 h-3.5" />}
                    <span className="hidden sm:inline">{activeContact.is_bot_paused ? 'Aktifkan AI' : 'Pause AI'}</span>
                  </button>
                  <button
                  ...
                  >
                    <Phone className={`w-4 h-4 ${requestingPhone ? 'animate-pulse' : ''}`} />
                  </button>
                  <div className="relative" ref={menuRef}>
                    ...
                    <AnimatePresence>
                      {showContactMenu && (
                        ...
                        >
                          <button onClick={openLabelModal} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-slate-200 hover:bg-slate-800 transition-colors">
                            <Tag className="w-4 h-4 text-emerald-400" /> Kelola Label
                          </button>
                          ...
                          <button onClick={handleCopyContactId} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-slate-200 hover:bg-slate-800 transition-colors">
                            <Copy className="w-4 h-4 text-slate-400" /> Salin ID Kontak
                          </button>
                          <div className="border-t border-slate-700" />
                          <button onClick={handleClearChat} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-red-400 hover:bg-red-500/10 transition-colors">
                  ...
                  >
                    <div className="flex items-center gap-2 bg-slate-800/50 rounded-xl px-3 py-2">
                      ...
                      <div className="flex-1 min-w-0 border-l-2 border-blue-400 pl-2">
                        <p className="text-xs font-semibold text-blue-400">{replyTo.sender_name}</p>
                        <p className="text-xs text-slate-400 truncate">{formatPreview(replyTo.body)}</p>
                      </div>
              ...
              <div className="p-4 bg-slate-900/40 dark:bg-white backdrop-blur-xl border-t border-slate-800/50 dark:border-slate-200">
                <div className="relative flex items-end gap-2 bg-slate-950/50 dark:bg-slate-50 border border-slate-700/50 dark:border-slate-300 rounded-2xl p-2 focus-within:border-blue-500/50 transition-colors">
                  <button
                    onClick={() => { setShowMediaPicker(true); fetchMediaAssets(); }}
                    disabled={sending}
                ...
                <p className="text-[10px] text-slate-500 text-center mt-2">
                  Kirim pesan/media manual akan otomatis pause AI selama 30 menit. Double-click pesan untuk reply.
                </p>
              </div>
            </>
          ) : (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-slate-700">
                ...
                <button onClick={() => setShowMediaPicker(false)} className="text-slate-400 hover:text-white p-1">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="flex-1 overflow-y-auto custom-scrollbar p-4">
                {loadingMedia ? (
                  ...
                  <p className="text-center text-slate-500 py-8 text-sm">Tidak ada media terdaftar untuk agen toko ini.<br />Upload di menu Media Gallery.</p>
                ) : (
                    ...
                    {mediaAssets.map(asset => (
                      ...
                      >
                        ...
                        <div className="p-2 bg-slate-800/90">
                          <p className="text-xs font-semibold text-white truncate">{asset.label}</p>
                          {asset.description && <p className="text-[10px] text-slate-400 truncate">{asset.description}</p>}
                        </div>
                      </button>
                    ))}
                  </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-slate-700">
                ...
                <button onClick={() => setShowLabelModal(false)} className="text-slate-400 hover:text-white p-1">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-4 space-y-4">
                <p className="text-xs text-slate-500">
                  Label disinkronkan ke WhatsApp Business. Tap untuk tambah/hapus.
                </p>
                {loadingLabels ? (
                  ...
                  <p className="text-sm text-slate-500 text-center py-4">
                    Tidak ada label di WA Business. Buat label dulu di aplikasi WhatsApp.
                  </p>
                ) : (
                    ...
                    {waLabelsList.map((lbl) => {
                      ...
                      return (
                        <button
                          ...
                          style={lbl.hexColor && active ? { borderColor: lbl.hexColor } : undefined}
                        >
                          {lbl.name}
                        </button>
                      );
                ...
                >
                  <RotateCw className={`w-3.5 h-3.5 ${loadingLabels ? 'animate-spin' : ''}`} />
      ...
      <AnimatePresence>
        {showForwardPicker && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"
            onClick={() => { setShowForwardPicker(false); setForwardMsgId(null); }}
          >
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-slate-700">
                ...
                <button onClick={() => { setShowForwardPicker(false); setForwardMsgId(null); }} className="text-slate-400 hover:text-white p-1">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-4 space-y-3">
                <p className="text-xs text-slate-400">Masukkan ID kontak tujuan (contoh: 62812xxxx@c.us)</p>
                <input
                  type="text"
                  value={forwardTarget}
      ...
      <AnimatePresence>
        {showLabelManager && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"
            onClick={() => { setShowLabelManager(false); setEditingLabelId(null); }}
          >
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-slate-700">
                ...
                <button onClick={() => { setShowLabelManager(false); setEditingLabelId(null); }} className="text-slate-400 hover:text-white p-1">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
                ...
                <div className="bg-slate-800/50 rounded-xl p-3 space-y-2">
                  <p className="text-xs font-medium text-slate-400">Buat Label Baru</p>
                  <div className="flex gap-2">
                    ...
                    >
                      {Array.from({ length: 20 }, (_, i) => (
                        <option key={i} value={i}>Warna {i}</option>
                      ))}
                    </select>
                    <button
                ...
                <div>
                  <p className="text-xs font-medium text-slate-400 mb-2">Label Terdaftar di WA</p>
                  {waLabelsList.length === 0 ? (
                    <p className="text-sm text-slate-500 text-center py-4">Belum ada label.</p>
                  ) : (
                    <div className="space-y-1.5">
                      {waLabelsList.map((lbl: WaLabel) => {
                        const isProtected = lbl.name === 'Closing' || lbl.name === 'Cancel';
                        return editingLabelId === lbl.id ? (
                          <div key={lbl.id} className="flex gap-2 items-center bg-slate-800 rounded-lg p-2">
                            <input
                              type="text"
                            ...
                            <select
                              ...
                              onChange={(e) => setEditingLabelColor(Number(e.target.value))}
                            ...
                            <button onClick={handleEditLabel} disabled={loadingLabels} className="px-2 py-1 bg-emerald-600 text-white rounded text-xs font-medium">Simpan</button>
                            <button onClick={() => setEditingLabelId(null)} className="px-2 py-1 bg-slate-600 text-white rounded text-xs">Batal</button>
                          </div>
                        ) : (
                          <div key={lbl.id} className="flex items-center justify-between bg-slate-800/50 rounded-lg px-3 py-2">
                            <div className="flex items-center gap-2">
                              <span className="text-sm text-white">{lbl.name}</span>
                              <span className="text-xs text-slate-500">Warna {lbl.color ?? '-'}</span>
                              {isProtected && (
                                <span className="text-[9px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20">🔒 Protected</span>
                              )}
                            </div>
                            <div className="flex gap-1">
                              <button
                                onClick={() => { setEditingLabelId(lbl.id); setEditingLabelName(lbl.name); setEditingLabelColor(lbl.color ?? 0); }}
        ...
        {showSummaryModal && (
          ...
          >
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-4">
                <pre className="text-sm text-slate-300 whitespace-pre-wrap font-sans leading-relaxed">{contactSummary}</pre>
              </div>
            </motion.div>
          </motion.div>
        )}
