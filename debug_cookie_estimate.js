...
async function test() {
    ...
    // Construct the cookie exactly as seen in DevTools or just the minimal required fields
    const cookieUser = encodeURIComponent(JSON.stringify({ token: TOKEN }));
    ...
    try {
        ...
        if (data.JNE) {
            console.log("JNE Auth:");
            console.log(" - Base:", data.JNE.price || data.JNE.estimatedPrice);
        ...
        if (data.JT) {
            console.log("\nJT Auth:");
            console.log(" - Base:", data.JT.price || data.JT.estimatedPrice);
